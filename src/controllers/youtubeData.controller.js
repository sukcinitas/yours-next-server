...
const YoutubeDataController = {
  async search(req, res) {
    try {
      ...
      const { data } = await YoutubeDataService.search({
        phrase: q,
        pageToken,
      });
  ...
  async getPlaylists(req, res) {
    try {
      ...
      const { data } = await YoutubeDataService.getPlaylists({
        channelId,
        pageToken,
      });
      return res.json({ success: true, data });
  ...
  async getPlaylistItems(req, res) {
    try {
      ...
      const pageToken = req.query.pageToken || '';
      const { data } = await YoutubeDataService.getPlaylistItems({
        playlistId,
        pageToken,
      });
      return res.json({ success: true, data });
  ...
  async getVideos(req, res) {
    try {
      ...
      const pageToken = req.query.pageToken || '';
      const { data } = await YoutubeDataService.getVideos({
        idList,
        pageToken,
      });
      return res.json({ success: true, data });
